"""
Transformer model: RoBERTa for sentiment analysis
"""
...
from models.transformers.bert import IMDBDataset, compute_metrics
...
def train_roberta(train_df, test_df, model_name="roberta-base",
                  batch_size=16, num_epochs=3, learning_rate=2e-5):
    """
    Train RoBERTa model for sentiment analysis.
    ...
    # Training arguments
    output_dir = Path(__file__).parent.parent.parent / "results" / "models" / "roberta"
    ...
    metrics = evaluate_model(
        y_test, y_test_pred,
        "RoBERTa-base",
        save_path="results/metrics/roberta.json"
    )
    
    # Save predictions
    save_predictions(y_test_pred, y_test, "roberta")
...
if __name__ == "__main__":
    ...
    model, tokenizer, metrics = train_roberta(train_df, test_df)
